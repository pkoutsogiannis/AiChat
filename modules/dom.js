...
export const dom = {
    ...
    $: (selector, attachHook = false) => {
        ...
        // not used - attachHookWithDelegate is always called
        const attachHookWithNoDelegate = (element) => {
            if (element && !element.hook && attachHook) {
                element.hook = function (event, callback) {
        ...
        if (elements.length === 1 && selector.startsWith('#')) {
        ...
        elementsList.forEach(attachHookWithDelegate);
