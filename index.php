 ...
 * ---------------------
 * The JavaScript modules can be bundled using bundle.js:
 * 
 *    node bundle.js [sourceDir="modules"]
 * 
 * The script will combine all JS files from sourceDir and xreate an app.js bundle
 * If terser is installed (npm install -g terser) the bundle will also be minified and a source map will be generated
 * 
 * Set $bundle_js accordingly.
 */
...
const JS_BUNDLE_NONE = 0; 	// Load individual JS files
const JS_BUNDLE_ONLY = 1; 	// Load only the bundle
const JS_BUNDLE_MINIFY = 2;	// Load the minified bundle

$bundle_js = JS_BUNDLE_NONE;
?>
...
<head>
	...
	<script type="module">
		import {
			app
		} from '<?= (($bundle_js == JS_BUNDLE_NONE ? "./modules/" : "./") . "app." . ($bundle_js == JS_BUNDLE_MINIFY ? "min." : "")) . "js?" . JS_VERSION ?>';
